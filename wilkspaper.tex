...
When LAMPN holds, it's possible to show that, asymptotically, the \emph{local state space} around $\rho_{0}$ converges to its \emph{tangent cone} $T(\rho_{0})$. This implies that the distribution of MLEs that we would obtain by solving the optimization problem in Equation \eqref{eq:mle} is the same as that which we would obtain by taking the \emph{unconstrained} MLEs and projecting them onto $T(\rho_{0})$:
\[\rhoML{d} = \underset{\rho \in T(\rho_{0})}{\text{argmin}}~||\rho  -\hat{\rho}||^{2}\]
where the unconstrained MLEs, satisfying LAN, are distributed around $\rho_{0}$ in $\M'$ as
\[\hat{\rho}\sim \mathcal{N}\left(\rho_{0}, \Id\right)\]
In the convex optimization community, $\rhoML{d}$ is known as the \emph{metric projection} of $\hat{\rho}$ onto the cone $T(\rho_{0})$ \cite{McCoy2014}. \todo{More clarifying text?}
Further,  it's possible to derive an exact expression for $\lambda(\M_{d}, \M_{d+1})$:
...
The following subsection presents the technical details and definitions to show the above results. The reader may skip this section without loss of continuity, and proceed to section \todo{????}.
