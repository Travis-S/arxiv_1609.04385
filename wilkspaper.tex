...
In a classical statistics, the null theory for $\lambda$ relies on the fact that the MLE has the local asymptotic normality property. However, in state tomography, $\rhoMLE$ will often \emph{not} have the this property. Why? Because of the positivity constraint, $\rhoMLE \geq 0$. This constraint distorts the distribution of estimates, even in the asymptotic limit, precisely when $\rho_{0}$ is rank-deficient within $\cH_{d}$. (That is, when $\rho_{0}$ is on the boundary of $\cH_{d}$. See Figure \ref{fig:boundaries}.) 

It should be noted that when $\rho_{0}$ is full-rank and has eigenvalues which are substantially larger than $\epsilon$,  local asymptotic normality will hold, meaning we may invoke the Wilks Theorem to do model selection. However if $\rho_{0}$ is rank-deficient, then the boundary looms, and $\mathrm{Pr}(\rhoMLE)$ is not Gaussian, meaning local asymptotic normality does not hold and the Wilks Theorem does not apply! Crucially, \emph{even if $\rho_{0}$ is full-rank in $\M_{d}$, it will be rank-deficient in $\M_{d+1}$}.

Many other authors have recognized this fact \todo[inline]{more references}
While the behavior of unconstrained MLE is well-studied classically, the behavior of the estimator near boundaries is not. \todo[inline]{WHY??}
...
To do so, we start by making assumptions about the \emph{unconstrained} MLE, denoted by $\rhohat$. Because it is unconstrained, this estimator will have the local asymptotic normality property. We will focus on the special and simple case where $\Fi$ at $\rho_{0}$ is  proportional to the Hilbert-Schmidt metric, so the likelihood function (and the distribution of $\rhohat$) is given by 
\begin{align}
\label{eq:likelihood}
\cL(\rho) & \propto e^{-\mathrm{Tr}[(\rho - \rhohat)^{2}]/2\epsilon^{2}}\\
\mathrm{Pr}(\rhohat) &\propto e^{-\mathrm{Tr}[(\rho_{0} - \rhohat)^{2}]/2\epsilon^{2}}
\end{align}
for some $\epsilon$ that scales as $1 / \sqrt{N_{\mathrm{samples}}}$.  In practice, $\Fi$  depends  on $\rho_{0}$ and the particular tomographic measurement performed, but the interaction of an arbitrary Fisher information with boundaries \footnote{By which we mean how the distribution of $\rhoMLE$ differs from that of $\rhohat$.} is complex and intractable. The isotropic assumption greatly simplifies our study of the problem, and permits the derivation of analytic results which capture realistic tomographic scenarios surprisingly well \cite{Smolin2012}. In classical statistics, the fact that the models considered typically do not have boundaries allows for the execution of a co\"{o}rdinate transformation to bring the Fisher information into an isotropic form. While it is possible to do such a transformation here, the resulting geometry of the state space (i.e., what $\rhoMLE \geq 0$ looks like) would be difficult to work with. 
...
\todo[inline]{Make this a bit more rigorous}
\todo[inline]{A figure showing the Pythagorean theorem}
