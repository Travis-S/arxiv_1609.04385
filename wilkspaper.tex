...
\textbf{Definition: Local Asymptotic Metric-Projected Normality (LAMPN)}

Given a statistical model $\M$ for which LAN does not hold, $\M$ is said to satisfy LAMPN when the following conditions both hold:
\todo[inline]{Does $\M$ need to be \emph{bounded} in some way?}
\begin{itemize}
\item[(a)] There exists a larger model $\M'$ such that $\M$ may be written as a submanifold of it: $\M \subset \M'$.
\item[(b)] For each true state $\rho_{0} \in \M$, when $\rho_{0}$ is embedded in $\M'$, LAN holds.
\end{itemize}

\todo[inline]{Explain differences b/t the two?}
...
\todo[inline]{Make sure this is actually correct, and shown in the next section}
...
\todo[inline]{Do we need more explanation here of why we're proving this?}
...
\begin{align*}
\end{align*}
\todo[inline]{Need to put some integrals in here.}
 
 See Figure ??? for a schematic of the geometry.
 \missingfigure{Geometry of the problem in HS coordinates}
 
Because $\M$ has boundaries it has a finite \emph{diameter} and in turn, a finite \emph{radius} $R$. \todo[inline]{Does this obviously follow from the idea of constraints? Could $\M$ be unbounded in some way?} If $\M$ is the quantum state space defined over a Hilbert space of dimension $d$, then the radius is the Hilbert-Schmidt distance from any pure state to $\Id_{d}/d$, which is $\sqrt{1-1/d}$.

 To summarize, in Hilbert-Schmidt coordinates, we have
 \begin{itemize}
 \item[1)] $\Sigma = N^{-1}_{\mathrm{samples}}\mathcal{I}^{-1}$
 \item[2)] $B$ has a radius $1/\sqrt{N_{\mathrm{samples}}e_{\min}}$
 \item[3)] $\M$ has radius $R$
 \item[4)] $\mathrm{Pr}\left(\rhoML{\M'} \in B\right) = 1$, asymptotically.
 \end{itemize}
 \todo[inline]{Is there some nicer way of presenting this material?}
Now, we switch to \emph{Fisher-adjusted} coordinates, so that the covariance matrix of the distribution of $\rhoML{\M'}$ is equal to the identity. This coordinate transformation may be obtained simply by mapping $\rho \rightarrow \Sigma^{-1/2}\rho~\forall~\rho \in \M, \M'$. (For any random variable $\bs{x}, \mathrm{Cov}(A\bs{x}) = A\mathrm{Cov}(\bs{x})A^{T}$ for any $A$.) Under this transformation, we obtain
\begin{align*}
\mathrm{Cov}(\Sigma^{-1/2}\rhoML{\M'})  &=\Sigma^{-1/2}\mathrm{Cov}(\rhoML{\M'})(\Sigma^{-1/2})^{T}\\
&= \Sigma^{1/2}(\Sigma^{-1/2})^{T} = \Id
\end{align*}

 Thus, in Fisher-adjusted coordinates, the covariance matrix of the MLEs is the identity: $\mathrm{Pr}(\rhoML{\M'}) \rightarrow \mathcal{N}(\Sigma^{-1/2}\rho_{0}, \Id)$.
...
\end{align*}
\todo[inline]{Show how $B$ changes, and radius of state space.}
...
\todo[inline]{Do we need more text about this? Finite sample effects + curvature?}
...
\emph{Case 1}: Assume $\rhoML{\M'} \not \in T(\rho_{0})$. Because $\rhoML{\M}$ is the closest point to $\rhoML{\M'}$ in $T(\rho_{0})$, and because $T(\rho_{0})$ contains $\rho_{0}$ (as its origin), the lines joining $\rho_{0}$ to $\rhoML{\M}$, and $\rhoML{\M}$ to $\rhoML{\M'}$, are at right angles to one another. (See Figure \ref{fig:tangentcone}.) \todo[inline]{more explanation for figure}
...
\todo[inline]{Maybe some words about how this result is useful for information criteria?}
\todo[inline]{Provide expression in non-Fisher adjusted coordinates - how to handle factor of $N_{\mathrm{samples}}$?}
...
With the framework of LAMPN in hand, we are ready to derive a replacement for the Wilks theorem. In deriving such a replacement, we face the obstacle that the distributions of $\rhoML{d}, \rhoML{d+1}$ are complicated, highly non-Gaussian, and singular (because estimates ``pile up'' on the various faces of the boundary; recall Figure \ref{fig:boundaries}).  For this reason, we will not attempt to compute $\mathrm{Pr}(\lambda)$ directly.  Instead, we focus on deriving a good approximation for $\langle \lambda \rangle$.
...
The Fisher information $\mathcal{I}_{k}$ is generally anisotropic (see Figure \ref{fig:anisofi}). And while the constraint $\rho\geq0$ that invalidated LAN in the first place is at least somewhat tractable in standard (Hilbert-Schmidt) coordinates, it becomes completely intractable in Fisher-adjusted coordinates.  So, to obtain a semi-analytic null theory for $\lambda$, we will simplify to the case where   $\mathcal{I}_{k} = \Id_{k}/\epsilon $ for some $\epsilon$ that scales as $1/\sqrt{N_{\mathrm{samples}}}$. (That is, $\mathcal{I}_{k}$ is proportional to the Hilbert-Schmidt metric.) The isotropic assumption greatly simplifies our study of the problem, and permits the derivation of analytic results which capture realistic tomographic scenarios surprisingly well \cite{Smolin2012}.
