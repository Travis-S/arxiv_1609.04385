...
\todo[inline]{Explain differences b/t LAN and LAMPN?}

If $\M$ satisfies LAMPN, then several nice results follow. The first is that, asymptotically, the \emph{local state space} around $\rho_{0}$ converges to the \emph{tangent cone} at $\rho_{0}$, denoted $T(\rho_{0})$. This, plus the fact that LAN holds in $\M'$, implies $\rhoML{\M}$ is the solution to the following optimization problem
\begin{equation}
\label{eq:lampnmle}
...
Second, when LAMPN is satisfied, the loglikelihood ratio $\lambda(\rho_{0}, \M)$, given by
...
\todo[inline]{Is this the right scaling?}
...
\todo[inline]{Should we put the optimization problem back into HS coordinates?}
